...
function DataManagementSection() {
  ...
  const [wipeCreds, setWipeCreds] = useState({ username: "", password: "" });
  ...
  const doBackup = async () => {
    try {
      await showLoader();
      const backup = await api.get<any>("/settings/backup");
      const blob = new Blob([JSON.stringify(backup, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `16eyes_backup_${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
  ...
  const doRestore = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        await api.post("/settings/restore", { data: data.data || data });
  ...
  const doWipe = async () => {
    if (!wipeCreds.username || !wipeCreds.password) {
      toast.error("Please enter verification credentials");
      return;
    }
    try {
      await showLoader();
      await api.post("/settings/wipe", wipeCreds);
      toast.success("Database wiped successfully");
      setShowWipe(false);
      window.location.reload();
    } catch (err: any) {
      toast.error(err.message || "Wipe failed: Incorrect credentials");
    }
  };
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="rounded-lg border border-dashed border-border p-4">
            <h3 className="text-sm font-semibold mb-1">Backup Data</h3>
            <p className="text-xs text-muted-foreground mb-4">Download a complete copy of your farmhouse data as a JSON file.</p>
            <button onClick={doBackup} className="rounded-md border border-navy px-4 py-2 text-xs font-medium text-navy hover:bg-navy/5">Download Backup</button>
          </div>

          <div className="rounded-lg border border-dashed border-border p-4">
            <h3 className="text-sm font-semibold mb-1">Restore Data</h3>
            <p className="text-xs text-muted-foreground mb-4">Import data from a previous backup file. This will merge with current data.</p>
            <label className="inline-block cursor-pointer rounded-md bg-navy px-4 py-2 text-xs font-medium text-white hover:bg-navy-hover">
              Upload Backup File
              <input type="file" accept=".json" onChange={doRestore} className="hidden" />
            </label>
          </div>
        </div>

        <div className="rounded-lg border border-danger/20 bg-danger/5 p-4">
          <h3 className="text-sm font-semibold text-danger mb-1 uppercase tracking-wider">Danger Zone</h3>
          <p className="text-xs text-muted-foreground mb-4">Wipe all data from the system (bookings, expenses, incomes). Your account will be kept.</p>
          <button onClick={() => setShowWipe(true)} className="rounded-md bg-danger px-4 py-2 text-xs font-medium text-white hover:bg-danger/90">Wipe All Data</button>
        </div>
      </div>

      <Modal open={showWipe} onClose={() => setShowWipe(false)} title="Verify Wipe Operation"
        footer={<><button onClick={() => setShowWipe(false)} className="rounded-md border border-border px-4 py-2 text-sm">Cancel</button><button onClick={doWipe} className="rounded-md bg-danger px-4 py-2 text-sm text-white">Confirm Wipe</button></>}>
        <div className="space-y-4">
          <div className="rounded-md bg-warning/10 p-3 border border-warning/20">
            <p className="text-xs text-warning-fg font-medium">⚠️ WARNING: This action will permanently delete all records. This cannot be undone.</p>
          </div>
          <p className="text-sm text-muted-foreground">Please enter your Admin username and password to proceed.</p>
          <Field label="Admin Username"><input value={wipeCreds.username} onChange={(e) => setWipeCreds({ ...wipeCreds, username: e.target.value })} className={inp}/></Field>
          <Field label="Admin Password"><input type="password" value={wipeCreds.password} onChange={(e) => setWipeCreds({ ...wipeCreds, password: e.target.value })} className={inp}/></Field>
        </div>
      </Modal>
    </Section>
  );
